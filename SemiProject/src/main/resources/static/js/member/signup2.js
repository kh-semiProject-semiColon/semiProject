...
memberEmail.addEventListener("input", (e) => {
  ...
  // 이메일 형식으로 작성하여 도메인 부분은 최소 2개의 문자로 끝나야한다 (.com, .net)
  ...
  if (!regExp.test(inputEmail)) {
    ...
    emailMessage.classList.remove("confirm"); // 초록색 글자 삭제
...
// 전공여부 유효성 검사
const majorYes = document.querySelector("#major-yes");
const majorNo = document.querySelector("#major-no");

majorYes.addEventListener("change", () => {
  checkObj.memberMajor = majorYes.checked || majorNo.checked;
});

majorNo.addEventListener("change", () => {
  checkObj.memberMajor = majorYes.checked || majorNo.checked;
});

if (!majorYes.checked && !majorNo.checked) {
  checkObj.memberMajor = false;
} else {
  checkObj.memberMajor = true;
}
