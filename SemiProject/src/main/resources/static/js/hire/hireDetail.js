...
function openResumeModal(memberNo) {
  ...
  fetch(`/hire/popup?memberNo=${memberNo}`)
    .then((res) => res.json())
    .then((data) => {
      document.getElementById("resumeNickname").textContent =
        data.memberNickname || "닉네임 없음";
      document.getElementById("resumeIntro").textContent =
        data.memberIntroduce || "자기소개 없음";
      document.getElementById("resumeProfileImg").src =
        data.profileImg || "/images/default_profile.png";
      document.getElementById("resumeMajor").textContent =
        data.memberMajor === "Y" ? "○" : "×";
