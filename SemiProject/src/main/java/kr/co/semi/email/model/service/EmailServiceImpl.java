package kr.co.semi.email.model.service;
...
import kr.co.semi.email.model.mapper.EmailMapper;
...
public class EmailServiceImpl implements EmailService{
	...
	private final JavaMailSender mailSender; // 실제 메일 발송을 담당하는 객체(EamilConfig 설정이 적용된 객체)
	...
	public String sendEmail(String htmlName, String email) {
		...
		// DB저장 시도 - 실패시 해당 메서드 종료
		if(!storeAuthKey(map)) {
			return null;
		}
		
		// DB 저장 성공된 경우에만 메일 발송 시도
		...
		try {
			...
			// - mimMessage : MimeMessage 객체로, 이메일 메시지의 내용을 담고있음
			...
			helper.setSubject("[boardProject] 회원 가입 인증 번호입니다"); // 제목
			helper.setText(loadHtml(authKey, htmlName), true); // 내용
	...
	// HTML 템플렛에 데이터를 바인딩하여 최종 HTML 생성 메서드
	private String loadHtml(String authKey, String htmlName) {
		...
		context.setVariable("authKey", authKey); // 템플릿에 사용할 변수 authKey에 값 설정
	...
	@Transactional(rollbackFor=Exception.class) // 메서드 레벨에서도 이용가능 (해당 메서드에서만 트랜잭션 커밋/롤백)
	private boolean storeAuthKey(Map<String, String> map) {
		...
		// 2. UPDATE 수행 시 0이 반환됨 == UPDATE 안됨 == INSERT된 내용이 없다
		// -> INSERT 수행
		if(result == 0) {
	...
	// 인증번호 발급 메서드
	// UUID를 사용하여 인증키 생성
	// UUID(Universally Unique IDentifier) : 전 세계에서 고유한 식별자를 생성하기 위한 표준 객체
	// 매우 낮은 확률로 중복되는 식별자를 생성한다 (어지간하면 안 겹친다고)
