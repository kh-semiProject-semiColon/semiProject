...
UPDATE "MEMBER" SET
MEMBER_PW = '$2a$10$uV/354JAegTV76W/7iD.z.UkUZwLJlnr4XEmiDbuAcIuib7D8ik9W'
WHERE MEMBER_NO = 2;

ALTER TABLE "HIRE_COMMENT" ADD HIRE_PARENT_COMMENT_NO NUMBER;
COMMENT ON COLUMN "ANNOUNCEMENT"."ANNOUNCE_UPDATE_DATE" IS '게시글 수정 날짜';

ALTER TABLE "HIRE_COMMENT" ADD CONSTRAINT "FK_HIRE_COMMENT_TO_HIRE_COMMENT_1" FOREIGN KEY (
	"HIRE_PARENT_COMMENT_NO"
)
REFERENCES "HIRE_COMMENT" (
	"HIRE_COMMENT_NO"
);
...
SELECT 
    ...
    sb.STUDY_BOARD_DEL_FL, 
    sc.STUDY_COMMENT_DEL_FL, 
    b.BOARD_DEL_FL, 
    c.COMMENT_DEL_FL
FROM "HIRE_INFO" h
JOIN "HIRE_COMMENT" hc ON h.MEMBER_NO = hc.MEMBER_NO
JOIN "STUDY_BOARD" sb ON h.MEMBER_NO = sb.MEMBER_NO
JOIN "STUDY_COMMENT" sc ON h.MEMBER_NO = sc.MEMBER_NO
JOIN "BOARD" b ON h.MEMBER_NO = b.MEMBER_NO
JOIN "COMMENT" c ON h.MEMBER_NO = c.MEMBER_NO
WHERE c.MEMBER_NO = 1;

* FROM "HIRE_INFO" WHERE MEMBER_NO = 1;
...
FROM (
  SELECT
    ...
    hc.HIRE_COMMENT_CONTENT AS commentContent,
    TO_CHAR(hc.HIRE_COMMENT_DATE, 'YYYY-MM-DD') AS commentDate,
    3 AS boardCode
  FROM HIRE_INFO h
  JOIN HIRE_COMMENT hc ON h.HIRE_NO = hc.HIRE_NO
  ...
  SELECT
    ...
    TO_CHAR(sc.STUDY_COMMENT_DATE, 'YYYY-MM-DD') AS commentDate,
    2 AS boardCode
  FROM STUDY_BOARD s
  JOIN STUDY_COMMENT sc ON s.STUDY_BOARD_NO = sc.STUDY_BOARD_NO
  WHERE sc.MEMBER_NO = 1
    AND s.STUDY_BOARD_DEL_FL = 'N'
  ...
  SELECT
    ...
    TO_CHAR(c.COMMENT_WRITTEN_DATE, 'YYYY-MM-DD') AS commentDate,
    1 AS boardCode
  FROM BOARD b
...
SELECT * FROM "BOARD"; -- BOARD_CODE가있음

SELECT * FROM "HIRE_INFO"; -- 게시글 번호만 있으면 됨
SELECT * FROM "STUDY_BOARD";  -- 게시글 번호만 있으면 됨
